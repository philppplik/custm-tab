...
permissions are added — searches navigate to public engine URLs only.| File | Role |
|------|------|
...
| `favicon.js` | `CUSTM_FAVICON`. Resolves a tile icon on-device, as a monogram, or via an opt-in third-party lookup. |
| `backgrounds.js` | `CUSTM_BACKGROUND`. Gradient palettes, flat colours with computed contrast, the photo layer, and the sunrise reveal. |
